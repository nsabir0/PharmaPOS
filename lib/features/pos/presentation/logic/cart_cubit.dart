...
class CartCubit extends Cubit<CartState> {
  ...
  CartCubit(this._salesRepository, this._db, this._inventoryCubit) : super(const CartState());

  void addToCart(Product product) {
    AppLogger.info('Adding product to cart: ${product.name} (ID: ${product.id})');
    ...
    if (currentInCart + 1 > product.stockQuantity) {
      AppLogger.warning('Insufficient stock for ${product.name}. Available: ${product.stockQuantity}');
      emit(state.copyWith(errorMessage: 'Not enough stock available for ${product.name}'));
    ...
    if (existingIndex >= 0) {
      ...
      AppLogger.debug('Updated quantity for ${product.name}. New quantity: ${updatedItems[existingIndex].quantity}');
    } else {
      emit(state.copyWith(
  ...
  void updateQuantity(int productId, int quantity) {
    ...
    if (itemIndex != -1) {
      ...
      if (quantity > item.product.stockQuantity) {
        emit(state.copyWith(errorMessage: 'Cannot exceed available stock (${item.product.stockQuantity})'));
        emit(state.copyWith(errorMessage: null));
        return;
      }

      AppLogger.info('Updating quantity for Product ID: $productId to $quantity');
  ...
  Future<void> checkout() async {
    ...
    AppLogger.info('Starting checkout process. Total Amount: ৳${state.totalAmount}');
    ...
    try {
      ...
      final saleId = await _db.createOfflineSale(
        ...
        state.items.map((item) => LocalSaleItemsCompanion(
      ...
      AppLogger.info('Sale saved locally. ID: $saleId. Attempting online sync...');
      ...
      result.fold(
        (failure) {
          AppLogger.warning('Online sync failed: ${failure.message}. Sale remains offline.');
      ...
      emit(state.copyWith(isSubmitting: false, errorMessage: 'Local database error: $e'));
    }
  }
}
